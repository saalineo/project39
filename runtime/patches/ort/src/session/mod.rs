...
impl Session {
	...
	fn run_inner_async<'i, 'r, 's: 'r, 'v: 'i + 's>(
		...
		let mut ctx = Box::new(core::mem::MaybeUninit::<AsyncInferenceContext>::uninit());
		unsafe {
			use core::ptr::write;

			let ctx_ptr = ctx.as_mut_ptr();
			write(core::ptr::addr_of_mut!((*ctx_ptr).inner), Arc::clone(&async_inner));
			...
			write(core::ptr::addr_of_mut!((*ctx_ptr).input_ort_values), input_ort_values);
			write(core::ptr::addr_of_mut!((*ctx_ptr)._input_inner_holders), input_inner_holders);
			write(core::ptr::addr_of_mut!((*ctx_ptr).input_name_ptrs), input_name_ptrs);
			write(core::ptr::addr_of_mut!((*ctx_ptr).output_name_ptrs), output_name_ptrs);
			write(core::ptr::addr_of_mut!((*ctx_ptr).output_names), output_names);
			write(core::ptr::addr_of_mut!((*ctx_ptr).output_value_ptrs), output_tensor_ptrs);
			write(core::ptr::addr_of_mut!((*ctx_ptr).session_inner), &self.inner);
		};
		let ctx = unsafe { Box::from_raw(Box::into_raw(ctx) as *mut AsyncInferenceContext) };
